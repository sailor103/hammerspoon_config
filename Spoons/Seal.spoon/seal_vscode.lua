...
local obj = {}
obj.__index = obj
obj.__name = "seal_vscode"

--- Seal.plugins.vscode.fileSearchPaths
--- Variable
...
--- Seal.plugins.vscode.maxResults
--- Variable
--- Maximum number of results to display
obj.maxQueryResults = 40

--- Seal.plugins.vscode.displayResultsTimeout
--- Variable
...
function obj:commands()
    return {
        vscode = {
            cmd = "cf",
            fn = obj.fileSearchByVSCode,
            name = "Open by VSCode",
            description = "Open by VSCode",
            plugin = obj.__name
        },
        webstorm = {
            cmd = "wf",
            fn = obj.fileSearchByWebStorm,
            name = "Open by WebStrom",
            description = "Open by WebStrom",
            plugin = obj.__name
        }
    }
end
...
function obj.completionCallback(rowInfo)
    if obj.openBy == BY_VS_CODE then
        hs.task.new("/usr/local/bin/code", nil, {rowInfo["path"]}):start()
    else
        hs.task.new("/Applications/WebStorm.app/Contents/MacOS/webstorm", nil, {rowInfo["path"]}):start()
    end

end

function obj.fileSearchByVSCode(query)
    obj.openBy = 'vscode'
    return obj.fileSearch(query)
end

function obj.fileSearchByWebStorm(query)
    obj.openBy = 'webstorm'
    return obj.fileSearch(query)
end
